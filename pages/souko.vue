  ...
  <v-container fluid>
    <v-data-iterator :items="posts" hide-default-footer>
      <template v-slot:header>
        <v-toolbar>
          <v-toolbar-title>ストレージ</v-toolbar-title>
          <v-spacer></v-spacer>
...
export default {
  ...
  mounted() {
    ...
    axios.get('/storage').then((response) => (this.posts = response.data))
    // axios.get('/api/storage').then((response) => (this.posts = response.data))
